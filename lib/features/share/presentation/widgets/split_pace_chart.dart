...
class SplitPaceChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 配速转换为分钟数（越低越快）
    final paces = splits.map((s) => s.paceSecPerKm / 60.0).toList();
        ...
        BarChartData(
            ...
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  ...
                  // 每 N 个显示一次，避免拥挤
                  final interval = splits.length > 15 ? 5 : (splits.length > 8 ? 2 : 1);
                  if (km == 1 || km % interval == 0 || km == splits.length) {
                    return Text('$km', style: const TextStyle(
                      fontSize: 8, color: SolrunColors.darkMuted,
          ...
          barGroups: List.generate(splits.length, (i) {
            ...
            final t = maxPace > minPace
                ? 1 - (paces[i] - minPace) / (maxPace - minPace) // 反转：低配速=快=1
                : 0.5;
